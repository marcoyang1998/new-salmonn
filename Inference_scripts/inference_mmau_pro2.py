...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate SALMONN on MMAU-Pro with native multi-audio prompts"
    )
...
def get_audio_label(index: int) -> str:
    if index < len(ORDINALS):
        return f"{ORDINALS[index]} audio"
    return f"audio {index + 1}"


def resolve_audio_paths(audio_value, audio_root: str) -> list[str]:
    if isinstance(audio_value, (str, os.PathLike)):
        rel_paths = [audio_value]
    else:
        rel_paths = list(audio_value)
    return [
        os.path.join(audio_root, os.fspath(path).lstrip("./"))
        for path in rel_paths
    ]


def build_multi_prompt(question: str, choices: list, num_audios: int, mc_prompt_style: str = "neutral") -> str:
    audio_lines = "\n".join(
        f"{get_audio_label(i).capitalize()}: <audio>" for i in range(num_audios)
    )
    choices_lines = "\n".join(
        f"Option {LETTERS[i]}: {choice}" for i, choice in enumerate(choices)
    )
    return (
        f"Listen to the following {num_audios} audio clips in order.\n"
        f"{audio_lines}\n"
        "Use all of the audio clips together to answer the following multiple-choice question.\n"
        f"Question: {question}\n"
        "Choices:\n"
        f"{choices_lines}\n"
        f"{get_mc_prompt_instruction(mc_prompt_style)}"
    )
...
def get_fbank(model_args):
    ...
    if enc not in ZIPFORMER_LIKE:
        raise ValueError(
            "inference_mmau_pro2.py only supports encoder_type in "
            f"{sorted(ZIPFORMER_LIKE)}, but got {enc!r}."
        )
    return Fbank(FbankConfig(num_mel_bins=128))


def extract_features(audio_paths, fbank, model_args):
    """Extract features for one or more audio files, preserving file order."""
    enc = model_args.encoder_type
    if enc not in ZIPFORMER_LIKE:
        raise ValueError(
            "inference_mmau_pro2.py only supports encoder_type in "
    ...
    for audio_path in audio_paths:
        ...
        audio_nums.append(1)
        features.append(fbank.extract(audio.squeeze(), sampling_rate=fs))

    return features, audio_nums


def prepare_model_inputs(text, tokenizer, model, model_args):
...
def main():
    ...
    if model_args.encoder_type not in ZIPFORMER_LIKE:
        raise ValueError(
            "inference_mmau_pro2.py only supports encoder_type in "
            f"{sorted(ZIPFORMER_LIKE)}, but got {model_args.encoder_type!r}."
        )
    if getattr(model_args, "split_audio", False):
        print("MMAU-Pro inference uses native multi-audio inputs with split_audio=False; overriding split_audio to False.")
        model_args.split_audio = False
    ...
    for step, (i, row) in enumerate(new_rows, 1):
        ...
        raw_audio_paths = resolve_audio_paths(row["audio_path"], args.audio_root)
        ...
        if "<audio>" in prompt:
            user_content = prompt
            user_prompt = prompt.replace("<audio>", " ").strip()
        else:
            user_content = "<audio>" + prompt
            user_prompt = prompt
        ...
        feature, audio_nums = extract_features(
            audio_paths, fbank, model_args
        )

        audio_placeholder_count = user_content.count("<audio>")
        expected_audio_count = sum(audio_nums) if audio_nums else len(feature)
        if audio_placeholder_count != expected_audio_count:
            raise ValueError(
                f"Audio placeholder count ({audio_placeholder_count}) does not match "
                f"loaded audio feature count ({expected_audio_count}): {raw_audio_paths}"
            )
        ...
        model_inputs = prepare_model_inputs(text, tokenizer, model, model_args)

...
PROMPT_TEMPLATE = (
    "Identify the bird species in this audio clip.\n"
    "Choose exactly one label from the BEANS CBI bird-species label set below and answer with only that label.\n"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run SALMONN bird-species classification on BEANS CBI.")
    parser.add_argument("--model_name_or_path", type=str, required=True, help="Path to model checkpoint.")
    parser.add_argument(
        "--beans_cbi_json",
        type=str,
        default="data/beans_cbi_salmonn.json",
        help="Path to the BEANS CBI SALMONN-format JSON.",
    )
    parser.add_argument(
        "--output_path",
        type=str,
...
def main():
    ...
    with open(args.beans_cbi_json, "r") as f:
        payload = json.load(f)
    ...
    if not data:
        raise ValueError(f"No samples found in {args.beans_cbi_json} for split={args.split}")

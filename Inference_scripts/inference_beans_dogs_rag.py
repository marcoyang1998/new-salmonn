...
def parse_args():
    parser = argparse.ArgumentParser(description="Run retrieval-augmented SALMONN dog identity classification on BEANS dogs.")
    ...
    parser.add_argument(
        ...
        default="train",
        help="Dataset split used to build the retrieval DB.",
    )
    parser.add_argument(
        "--test_split",
        type=str,
...
def build_rag_prompt(base_prompt, rag_context):
    return (
        ...
        "Answer with only one BEANS dog identity label."
    )
...
def load_rag_db(
    ...
    saved_train_split = str(db["train_split"][0])
    saved_rag_embedding_backend = str(db["rag_embedding_backend"][0])
    if saved_train_split != train_split:
        raise ValueError(f"RAG DB train split {saved_train_split} does not match requested split {train_split}.")
...
def main():
    ...
    train_data = select_split(all_samples, args.train_split)
    if not train_data:
        raise ValueError(f"No retrieval samples found in {args.beans_dogs_json} for train_split={args.train_split}")

    test_data = select_split(all_samples, args.test_split)
    if args.limit is not None:
        test_data = test_data[: args.limit]
    if not test_data:
        raise ValueError(f"No test samples found in {args.beans_dogs_json} for test_split={args.test_split}")
    ...
    if args.rag_db_path and os.path.exists(args.rag_db_path):
        ...
        print(f"Building RAG DB from split {args.train_split}")
        label_to_index = get_label_to_index(payload if isinstance(payload, dict) else {"data": all_samples})
    ...
    print("\n===== BEANS Dogs RAG Inference Summary =====")

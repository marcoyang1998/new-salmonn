...
DEFAULT_PROMPT = "Describe the emotion of the speaker in one word."


def parse_args():
    parser = argparse.ArgumentParser(
        description="Run retrieval-augmented SALMONN emotion classification on IEMOCAP."
    )
    parser.add_argument("--model_name_or_path", type=str, required=True, help="Path to model checkpoint.")
    parser.add_argument(
        "--train_json",
        type=str,
        default="data/iemocap_emotion_train_salmonn.json",
        help="Path to the IEMOCAP train SALMONN-format JSON.",
    )
    parser.add_argument(
        "--test_json",
        type=str,
        default="data/iemocap_emotion_test_salmonn.json",
        help="Path to the IEMOCAP test SALMONN-format JSON.",
    )
    parser.add_argument(
        "--output_path",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Prompt text to use for IEMOCAP RAG inference.",
    )
...
def build_rag_prompt(base_prompt, rag_context):
    return (
        ...
        "Answer with only one IEMOCAP emotion label."
    )
...
def validate_label_maps(train_payload, test_payload):
    ...
    if train_labels != test_labels:
        raise ValueError("Train and test label sets do not match. Use matching IEMOCAP split JSON files.")
    return train_labels
...
def main():
    ...
    base_prompt = build_base_prompt(args.prompt)

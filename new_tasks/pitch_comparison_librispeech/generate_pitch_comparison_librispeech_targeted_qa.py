#!/usr/bin/env python3
"""Generate targeted LibriSpeech pitch-comparison QA samples in SALMONN format."""
...
from generate_pitch_comparison_librispeech_description_qa import (
    DEFAULT_AUDIO_PATH_PATTERN,
...
SOURCE_NAME = "pitch_comparison_librispeech_targeted_qa"
DEFAULT_OUT_DIR = Path("/data/milsrg1/huggingface/cache/xy316/pitch_comparison_librispeech_targeted_qa")
DEFAULT_NUM_SAMPLES = 10000

QUESTION_TEMPLATES = {
    "highest": [
        "Which segment has the highest pitch?",
        "Which part of the recording is spoken at the highest pitch?",
    ],
    "lowest": [
        "Which segment has the lowest pitch?",
        "Which part of the recording is spoken at the lowest pitch?",
    ],
    "relative": [
        "Which segment has a higher pitch, the {x} or the {y}?",
        "Is the {x} segment higher-pitched than the {y} segment?",
    ],
}
...
def build_highest_qa(levels: Sequence[str], rng: random.Random) -> Tuple[str, str, Dict[str, object]]:
    target_index = levels.index("high")
    question = rng.choice(QUESTION_TEMPLATES["highest"])
    answer = rng.choice(
        [
            f"{segment_phrase(target_index).capitalize()} has the highest pitch.",
            f"{part_phrase(target_index).capitalize()} is spoken at the highest pitch.",
            f"The highest-pitched portion is {segment_phrase(target_index)}.",
        ]
    )
    return question, answer, {"target_segment_index": target_index + 1}


def build_lowest_qa(levels: Sequence[str], rng: random.Random) -> Tuple[str, str, Dict[str, object]]:
    target_index = levels.index("low")
    question = rng.choice(QUESTION_TEMPLATES["lowest"])
    answer = rng.choice(
        [
            f"{segment_phrase(target_index).capitalize()} has the lowest pitch.",
            f"{part_phrase(target_index).capitalize()} is spoken at the lowest pitch.",
            f"The lowest-pitched portion is {segment_phrase(target_index)}.",
        ]
    )
    return question, answer, {"target_segment_index": target_index + 1}
...
def build_relative_qa(levels: Sequence[str], rng: random.Random) -> Tuple[str, str, Dict[str, object]]:
    ...
    x_is_higher = LEVEL_RANKS[x_level] > LEVEL_RANKS[y_level]
    higher_index = x_index if x_is_higher else y_index
    ...
    if template.startswith("Is"):
        if x_is_higher:
            answer = f"Yes. {segment_phrase(x_index).capitalize()} has a higher pitch than {segment_phrase(y_index)}."
        else:
            answer = f"No. {segment_phrase(y_index).capitalize()} has a higher pitch than {segment_phrase(x_index)}."
    else:
        answer = rng.choice(
            [
                f"{segment_phrase(higher_index).capitalize()} has the higher pitch.",
                f"The higher-pitched one is {segment_phrase(higher_index)}.",
                f"{segment_phrase(higher_index).capitalize()} is higher-pitched.",
            ]
        )
    ...
    return question, answer, {
        ...
        "x_is_higher": x_is_higher,
        "target_segment_index": higher_index + 1,
    }


def build_question_answer(pattern: str, rng: random.Random) -> Tuple[str, str, str, Dict[str, object]]:
    levels = pattern.split("-")
    question_type = rng.choice(["highest", "lowest", "relative"])
    if question_type == "highest":
        question, answer, metadata = build_highest_qa(levels, rng)
    elif question_type == "lowest":
        question, answer, metadata = build_lowest_qa(levels, rng)
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-json-name", default="pitch_comparison_librispeech_targeted_qa_10k.json")
